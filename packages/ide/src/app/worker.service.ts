...
export class WorkerService {
  ...
  async transpileArduinoCode(code: string): Promise<{
    c: string;
    errors: PortugolCodeError[];
    parseErrors: PortugolCodeError[];
    times: { parse: number; check: number; transpile: number };
    ...
    return new Promise(resolve => {
      ...
      const listener = (e: MessageEvent) => {
        if (e.data.id === id) {
          console.log("Arduino Transpiler Result", e.data, `${Date.now() - now}ms`);
          this.worker?.removeEventListener("message", listener);
          this.busy = false;
          resolve(e.data);
        }
      ...
      this.worker?.addEventListener("message", listener);
      this.worker?.postMessage({ code, id, action: "transpile-arduino" });
    });
  }
